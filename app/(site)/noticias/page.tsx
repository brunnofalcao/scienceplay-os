  ...
  return (
    <div className="wrap py-12">
      ...
      <header className="max-w-3xl">
        <span className="kicker">Saúde intestinal, em linguagem de gente</span>
        <h1 className="mt-3 text-[38px] leading-[1.08]">Notícias</h1>
        <p className="prose-5r mt-3">
          Reportagens sobre saúde intestinal, microbiota, alimentação e comportamento — a ciência como ponto
          de partida, escrita para você entender e usar no dia a dia. A base de cada matéria fica visível ao
          final, com o nível de evidência e as fontes.
        </p>
      </header>
      ...
      <section className="mt-10">
        <Link href={`/noticias/${today.slug}`} className="group grid gap-6 overflow-hidden rounded-2xl border border-line bg-white md:grid-cols-2">
          <div className="bg-navy-deep p-8 text-white">
            ...
            <h2 className="mt-3 font-serif text-[27px] leading-tight text-white group-hover:text-mist">{today.headline}</h2>
            <p className="mt-3 text-[15px] text-mist/85">{today.subheadline}</p>
          </div>
          <div className="flex flex-col justify-center p-8">
            ...
            <p className="mt-4 text-[14px] text-ink/70">{today.lead.slice(0, 180)}…</p>
            <span className="mt-4 font-medium text-navy group-hover:underline">Ler a matéria completa →</span>
          </div>
        </Link>
      </section>
            ...
            {rest.map((n) => (
              <li key={n.slug}>
                <Link href={`/noticias/${n.slug}`} className="card group flex h-full flex-col transition-shadow hover:shadow-md">
                  ...
                  <h3 className="mt-1.5 font-serif text-[18px] leading-snug text-navy-deep group-hover:text-navy">{n.headline}</h3>
                  <p className="mt-2 flex-1 text-[13.5px] leading-snug text-ink/65">{n.subheadline.slice(0, 110)}…</p>
